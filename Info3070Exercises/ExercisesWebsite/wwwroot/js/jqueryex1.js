$(function () { //main jQuery routine- executes every on page load
    ...
    $("#loadbutton").click(e => {
        ...
        //use const when data shouldn't mutated
        const studentData = JSON.parse(stringData);

        //we'll manually build a string of html. WE use let because the string will be mutated
        let html = "";

        //using the array map operator here to iterate through the object array
        //for each object it finds we'll label it as student and then dump
        //out the 3 properties using a template string inside a div node
